...
A newspaper-styled terminal UI for following MLB games, in the
...
order, season player stats, and headlines for however many teams you
follow.
...
search by team name/city. Division groups start collapsed; `enter`
expands one, or follows/unfollows a team. `escape` asks you to confirm
(y/n) before exiting, listing everything you're following.
...
├── mlb_api.py              # MLB Stats API client (async) — scores, box score, roster stats
...
├── screens/
│   ├── team_picker.py      # MLB team picker: browse by division, or search
...
Scores, box scores, and roster stats come from the MLB Stats API
